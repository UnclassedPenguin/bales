...
func main() {
  ...
  flag.BoolVar(        &list,        "l", false, "Prints the Database to terminal. Optionally you can " +
                                                 "use -g, -s, -r, -y, -m, -date...")
  flag.BoolVar(        &test,        "t", false, "If set, uses the test database.")
  flag.BoolVar(         &add,        "a", false, "Adds a record to the database. If set, requires -g " +
                                                 "(group) and -n (number of bales).")
  flag.BoolVar(         &del,        "d", false, "Deletes a record from the database. If set, requires " +
                                                 "-n (id number of entry to delete).")
  flag.BoolVar(      &square,        "s", false, "If set, indicates that the bale is square. Round is the " +
                                                 "default. This can be used when adding (-a) a record, or " +
                                                 "when listing (-l) to specify that you only want to see " +
                                                 "square bales.")
  flag.BoolVar(       &round,        "r", false, "If set, indicates that the bale is round. Round is the " +
                                                 "default. This can be used when adding (-a) a record, or i " +
                                                 "when listing (-l) to specify that you only want to see " +
                                                 "round bales.")
  ...
  flag.StringVar(      &year,     "year",    "", "Year to list from database. Can be a single year(ie 2019) " +
                                                 "or a range (ie 2019-2022)")
  flag.StringVar(     &month,    "month",    "", "Month to list from database. Can be a single month(ie 09) " +
                                                 "or a range (ie 09-12). Single digit months require a " +
                                                 "leading 0.")
  flag.StringVar(       &day,      "day",    "", "day to list from database. Can be a single day(ie 19) or " +
                                                 "a range (ie 09-30)")
  flag.StringVar(      &date,     "date",    "", "The date to put into the database, if not today. yyyy-mm-dd")
  flag.StringVar(  &dateFrom,     "from",    "", "List from specified date to current date. Date must be " +
                                                 "yyyy-mm-dd requires -l")
  flag.StringVar(    &custom,        "c",    "", "Custom SQL request. Requires -l. Example:\nbales -t -l -c " +
                                                 "\"SELECT * FROM bales WHERE strftime('%d', date) BETWEEN " +
                                                 "'01' AND '03'\"")
